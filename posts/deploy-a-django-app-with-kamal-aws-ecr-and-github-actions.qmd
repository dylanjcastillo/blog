...
description-meta: "A guide to deploy a Django app with Kamal, AWS ECR, and Github Actions"
toc: true
toc-depth: 3
lightbox: true
...
- Have an [AWS](https://aws.amazon.com/) account and [CLI](https://docs.aws.amazon.com/cli/latest/userguide/install-cliv2.html) installed.
...
docker network create --driver bridge kamal_network # this is only required if you're planning to run SQLite or redis on the same VPS
...
3. Exposes port 8000 and runs the application by executing the `entrypoint.sh` script. By exposing the port, Kamal will automatically detect that that is the port the app runs on and [will use that to set up the reverse proxy](https://github.com/basecamp/kamal/issues/58).
...
This will set up a reverse proxy using Traefik (with automatic SSL certificates using Let's Encrypt), a Redis database, and a volume for the SQLite database. Remember to replace the placeholders with your own values.
