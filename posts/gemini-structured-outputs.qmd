...
In this article, I'll share the results and a few things I learned while running the benchmarks.
...
1. Gemini's structured outputs performed on par with unstructured outputs.^[Assuming “structured outputs” refers to any method that generates LLM output adhering to a JSON schema.]
2. When testing constrained decoding specifically, Gemini’s structured outputs performed worse than unstructured outputs.
3. Function calling and constrained decoding have a big problem: the order of the keys specified in the schema is not preserved when using the [Generative AI Python SDK](https://ai.google.dev/). This will likely break chain-of-thought reasoning in your applications.
4. For constrained decoding, the model will reorder the keys alphabetically, so a quick fix is to name the keys in a way that preserves the chain-of-thought reasoning. I couldn't find a way to fix this issue in Gemini's function calling.
...
Similar to my [previous post](https://dylancastillo.co/posts/say-what-you-mean-sometimes.html), the results are also mixed for Gemini 1.5 Flash. Structured outputs perform on par with unstructured outputs. But only if you take the less rigid interpretation of "structured outputs".

Either way, you shouldn't be disappointed by these results. This only reinforces my position that you shouldn’t blindly trust random posts or papers. If you want to improve your LLM workflows, you need to test things for yourself.
