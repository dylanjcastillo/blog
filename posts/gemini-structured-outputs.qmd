---
title: "Evaluating the performance of Gemini's structured outputs"
date: "12/27/2024"
description-meta: "Is there a performance gap between structured and unstructured outputs when using Gemini 1.5 Flash? Let's find out."
categories:
  - llm
  - gemini
  - pydantic
...
Most companies I've worked with are using some form of structured outputs when working with LLMs. But very few of them have actually tested if going with structured outputs has made a difference in performance compared to unstructured outputs.

After reading [Let Me Speak Freely?](https://arxiv.org/abs/2408.02442) and [.txt's rebuttal](https://blog.dottxt.co/say-what-you-mean.html), I decided to run some benchmarks myself using popular proprietary models. This is a key component of my job, so I need to make sure the advice I give to clients is backed by data.

I previously published results for [_GPT-4o-mini_](https://dylancastillo.co/posts/say-what-you-mean-sometimes.html). Now, I've run the same benchmarks to _Gemini 1.5 Flash_ and wrote this article to share the results.

You can run the benchmarks yourself using [this notebook](https://github.com/dylanjcastillo/blog/blob/main/_extras/gemini-structured-outputs/gemini-structured-outputs-benchmarks.ipynb).
...
1. Gemini's structured outputs perform on par with unstructured outputs on the benchmarks considered.^[Assuming “structured outputs” refers to any method that generates LLM output adhering to a JSON schema.]
2. When constrained decoding is used specifically, Gemini’s structured outputs perform worse than unstructured outputs in the tasks considered.
3. The order of the keys specified in the schema will not be preserved when using Gemini's function calling or constrained decoding through the [Generative AI Python SDK](https://ai.google.dev/). If you rely on chain-of-thought reasoning and don't account for key order, this will invalidate it.
4. There is no obvious way to fix this issue in Gemini's function calling. During constrained decoding, the model will reorder the keys alphabetically, so you should name the keys in a way that preserves the chain-of-thought reasoning.
...
When considering both **JSON-Prompt** and **JSON-Schema**, Gemini's structured outputs perform comparably to its unstructured outputs. However, with **JSON-Schema** alone (i.e., constrained decoding), performance drops compared to unstructured outputs. This difference is most evident in the _Shuffled Objects_ task, where **NL** achieved a score of 97.15%, while **JSON-Schema** scored 86.18%.
...
When running the three benchmarks, I quickly ran into a big performance issue with _FC_ and _JSON-Schema_. In all tasks, both showed double-digit performance drops compared to NL.
...
After a bit of tinkering with the schema, I realized that the performance drop in _JSON-Schema_ was due to the keys in the schema being reversed when generating the response. Tam et al. mentioned briefly in Let Me Speak Freely? that _JSON-Schema_ responses failed to produce valid JSON due to this exact problem, so they excluded it from their analysis.

Through testing, I found that the order of the keys in the schema gets sorted alphabetically before the response is generated. To confirm this, I ran a test with 100 randomly generated schemas. Every resulting response had its keys sorted alphabetically, so it's clear this is by design.

The Vertex AI documentation mentions a `propertyOrdering` parameter, which should allow control over the order of keys. However, this feature [doesn’t appear](https://discuss.ai.google.dev/t/issue-with-key-ordering-in-controlled-generation-on-gemini-1-5/41737/3) [to work](https://github.com/google-gemini/generative-ai-python/issues/533) with the Generative AI Python SDK.

Since I couldn’t directly control the key order, I resorted to a quick workaround. I named the keys in a way that forced the desired order alphabetically. Instead of using `reasoning` and `answer`, I used `reasoning` and `solution`. This preserved the chain-of-thought reasoning in the responses, and resolved the performance drop in _JSON-Schema_.

_FC_ was a different story. Unlike _JSON-Schema_, the order of the keys seemed random, and I couldn’t find a clear pattern or a reliable way to control it. Due to this issue, I decided to exclude _FC_ from the analysis.
...
_NL_ and _JSON-Prompt_ behave similarly, without a clear winner between them. Each method got a slight edge over the other in 3 of the tasks. On the other hand, _JSON-Schema_ performed worst than _NL_ in 5 out of 6 tasks. Plus, in _Shuffled Objects_, it did so with a gap of more than 10pp: 97.15% for _NL_ vs. 86.18% for _JSON-Schema_.

Tam et al. interpreted structured outputs as any method that "involves providing output in standardized formats like JSON or XML through format restriction." Using this interpretation to analyze the results, there is no performance gap between structured and unstructured outputs. This contradicts the study's original claim.

But if you take the common interpretation that constrained decoding is the only form of structured generation, then the study's original conclusion still applies: there is indeed a significant performance gap between structured and unstructured outputs.

So, yes, it depends.

## Conclusion and recommendations

Similar to my [previous post](https://dylancastillo.co/posts/say-what-you-mean-sometimes.html), the results are also mixed for Gemini 1.5 Flash. Structured outputs perform on par with unstructured outputs. But only if you're taking the less rigid interpretation of "structured outputs". Otherwise, there's a difference in performance.

But you shouldn't be disappointed by these results. This only reinforces my position that you shouldn’t blindly trust random posts or papers. If you want to improve your LLM workflows, you need to test things for yourself.
