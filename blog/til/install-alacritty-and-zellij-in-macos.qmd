...
When I say hacker, I mean having a cool-looking terminal. The kind that makes people think I’m a stealing millions from banks when, in reality, I’m just struggling to read a JSON file.
...
You can install them using [Homebrew Cask](https://brew.sh/cask/).
...
Zellij is an improved version of [`tmux`](https://en.wikipedia.org/wiki/Tmux). Whenever I had to do something in `tmux`, half of the time it took me to finish the task was spent figuring out the key bindings. Zellij shows you (if you don't disable it) the key bindings on screen, which is a great improvement to the user experience. 
...
[Neovim](https://neovim.io/) is a fork from [Vim](https://www.vim.org/), that solves some of Vim's issues[^Including its horrible website.]. I don't really know if it's better or worse than Vim, but given that DHH recommended it, I thought it was a good idea to give it a shot.
...
Some of my favorite features that LazyVim enabled by default are:

- `<space><space>` to toggle which-key.
