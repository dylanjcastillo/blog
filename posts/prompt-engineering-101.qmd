...
I've tried every trick in the book to get Large Language Models (LLMs) to follow my instructions. I’ve resorted to threats of [physical violence](https://www.youtube.com/watch?v=8g7a0IWKDRE&t=500s). I’ve offered bribes. I’ve even made coding agents call me [big daddy](https://www.reddit.com/r/cursor/comments/1joapwk/comment/mkqg8aw) to ensure they follow my repo's rules^[It didn't work, but I felt much better about myself.].

All this trial and error has taught me a trick or two about writing prompts. This is a key part of using LLMs, but it's also one of the most hyped and abused techniques. There are so many AI influencers selling and sharing their bullet-proof prompting technique that it often feels closer to astrology than to engineering.

This article is a no-BS guide to help you get the basics right. It won't solve all the problems in your agentic workflow or LLM-based apps, but it will help you avoid making obvious mistakes.

Let's get started!

## What is a prompt?

A prompt is any text you send to an LLM to guide its behavior or generate a response. Most chat-based models work with two types of instructions:
...
2. **User prompt**: The actual question the end-user types and any additional context. Examples: “What’s today’s weather in Dublin?”; “Summarize the following documents.”
...
Then, these are passed through a [chat template](https://huggingface.co/docs/transformers/en/chat_templating) and converted into a single string that is sent to the model. For example, this is the resulting message text used by *Qwen3* after combining the system and user prompts:
...
After you make the request, the model will respond with an *assistant* message that contains the model's reply^[I removed the thinking tokens for brevity.]. It looks like this:
...
You'll generally only set the system and user prompts to instruct the model. But for some prompting techniques, such as [few-shot prompting](https://www.promptingguide.ai/techniques/fewshot), you might include assistant messages to simulate model responses.
...
2. Provide examples
3. Let models think
4. Structure prompts into sections and use delimiters
...
This information will tell the LLM its role and the specific task you want it to perform. This should go in the system prompt.
...
This technique is formally known as a "few-shot prompt." It's a simple but effective way to improve the [quality of the output](https://dylancastillo.co/posts/say-what-you-mean-sometimes.html#evaluating-gpt-4o-mini) in many tasks.
...
Input: 'Maria Julia Pedro Joe'
```

In my experience, it's better to provide these examples directly in the system prompt because it's easier to read, keeps everything close together, and — if the prompt is long enough — you'll often benefit from caching.
...
The headers are for reference only, you you can modify them or even skip them. You also don't need to include all of these sections in your prompt.
...
1. First, classify the document into a category.
2. Then, extract the entities from the document based on the category.
...
```text
You're an expert document classifier. Classify the document into the following categories:
...
LLMs often get confused when the context is too long and the instructions are complex. When the context exceeds a certain length, it's better to repeat the instructions at the bottom of the prompt. Anthropic has reported up to a [30% performance improvement](https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/overview#essential-tips-for-long-context-prompts) when using this technique.
...
1. **Exemplar Selection KNN (ES-KNN):** Instead of having a fixed selection of examples, you can embed the user query and the examples, and then use a KNN algorithm to select the most relevant examples. This has been shown to [improve the quality of results](https://arxiv.org/abs/2506.05614) in many tasks.
2. **Self-consistency (SC):** You use the model to generate multiple responses and then select the most consistent one by marginalizing over the noise. This has been shown to [boost performance](https://arxiv.org/abs/2203.11171) in CoT prompting.
3. **Thread of Thought (ThoT):** This is a [two-tiered prompting system](https://arxiv.org/abs/2311.08734) that first asks the LLM to do an analytical dissection of the context, step by step, and summarize intermediate results. Then, it uses another prompt to distill the analysis into a final answer.
...
These principles will not make your crappy LLM pipeline go from barely usable to raising millions from VC firms. But they will definitely get you on the right path. Or maybe not, who knows? People lie on the internet all the time. 
