...
Taking Tam et al.'s interpretation of what "structured outputs" means, the conclusion does not hold. In this case, there's no significant difference in performance between structured and unstructured outputs.

But if you take the interpretation that constrained decoding is the only way to generate structured outputs, then the conclusion does hold. In this case, there is a significant difference in performance between structured and unstructured outputs.

## Conclusion and recommendations

Similar to my [previous post](https://dylancastillo.co/posts/say-what-you-mean-sometimes.html), the results are also mixed for Gemini 1.5 Flash. Structured outputs perform on par with unstructured outputs. But, if you only consider constrained decoding, this is not the case.

This reinforces my position that you shouldn’t blindly trust random posts or papers. If you want to improve your LLM workflows, you need to test things for yourself.
...
It sucks when the answer is "it depends", but that's all I can tell you. So, run your own evals, and choose the approach that works best for your task.
