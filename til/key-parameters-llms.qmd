...
In the next sections, I'll go over each of these parameters in more detail and provide some suggestions about how to use them. Note that each provider has a slightly different name for the same parameter, but the concept is the same. You'll have to check the documentation of the provider you're using to see the exact name of the parameter.
...
This available for OpenAI models. Anthropic doesn't provide them. Gemini provides a [single request with logprobs per day](https://discuss.ai.google.dev/t/get-logprobs-at-output-token-level/54418/15) (yes, I'm not kidding 😅).
