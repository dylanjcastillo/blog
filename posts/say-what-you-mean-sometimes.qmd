...
This article presents the results of the first two steps. All the code is available on [Github](https://github.com/dylanjcastillo/blog/tree/main/extras/say-what-you-mean-sometimes).

## Results

In essence, Tam et al.’s conclusions about structured outputs might still hold, even if they did not properly test for it.

.txt's rebuttal is also correct, and shows that structured outputs are as good or better than unstructured outputs for _LLaMA3-8B-Instruct_. But the same results did not hold for _GPT-4o-mini_.
...
These retries primarily affected **FRI** results in the **GSM8K** and **Last Letter** datasets. But, since the other structured methods outperformed them, this adjustment does not alter the overall conclusions. The other methods maintained error rates of <0.5% in **GSM8K** and 0% in **Last Letter** and **Shuffle Object**.
...
On the other hand, **Shuffle Object** shows a clear advantage for unstructured outputs over structured outputs. This was unexpected, and even after adjusting the prompts, I couldn’t narrow the gap that much.

In the end, despite the issues in Tam et al.’s study, their conclusion appears to hold: there are scenarios where unstructured outputs are indeed better than structured ones.
...
I don’t think unstructured outputs are inherently better or worse than structured outputs across the board. It likely depends on your task, the model you're using, and your prompt engineering skills. You will need to test for yourself and find if there's a difference, and if so, which one performs better.
