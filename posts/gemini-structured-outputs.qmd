...
After a bit of tinkering with the schema, I realized that the performance drop in _JSON-Schema_ was due to the keys in the schema being reversed when generating the response. Tam et al. mentioned briefly in LMSF that _JSON-Schema_ responses failed to produce valid JSON due to this exact problem, so they excluded it from their analysis.

Through testing, I found that the order of the keys in the schema gets sorted alphabetically before the response is generated. To confirm this, I ran a test with 100 randomly generated schemas. Every resulting response had its keys sorted alphabetically. That's not good.
...
Since I couldn’t directly control the key order, I resorted to a quick hack. I named the keys in a way that forced the desired order alphabetically. Instead of using `reasoning` and `answer`, I used `reasoning` and `solution`. This preserved the chain-of-thought reasoning in the responses, and resolved the performance drop in _JSON-Schema_.
...
Similar to my [previous post](https://dylancastillo.co/posts/say-what-you-mean-sometimes.html), these results are mixed. Depending on how you interpret "structured outputs", you might find that there's no significant difference in performance between structured and unstructured outputs or that there is.

This only reinforces my position that you shouldn’t blindly trust random posts or papers. If you want to improve your LLM workflows, you need to test things for yourself.
...
This is a long ass post and sadly all I can tell you is what you likely presume from the beginning: run your own evals, then choose the approach that works best for your task.
